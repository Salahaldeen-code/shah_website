...
export function EditorialShowcase({ copy }: EditorialShowcaseProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const hasTriggeredRef = useRef(false);
  const reducedMotion = usePrefersReducedMotion();
  const [revealed, setRevealed] = useState(false);

  const { topText, middleText, bottomText, overlayText } = copy;
  const { colors, scrollAnimation, stepWidths } = editorialShowcaseConfig;
